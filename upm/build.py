...
from .db import visit_node, restore_node, check_db
...
def build_makefile_impl(node):
    ...
    def iter_nodes():
        for ptr in full:
            yield print_one_node(restore_node(ptr))
...
def print_one_node(root):
    ...
    def iter_part():
        ...
        yield target_name + ' ' + target + ': ' + ' '.join(gen_pkg_path(x) for x in iter_deps())
        ...
        def iter_body():
            ...
            for l in prepare_pkg('$(INSTALL_DIR)', '$(PKG_FILE)', root['node']()['codec']):
                yield l
