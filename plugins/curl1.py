...
@helper
def curl1(info):
    return curl0(info, [bestbox1(info), mbedtls1_dev(info), make1(info), tar1(info), xz1(info)], 'xz')


@helper
def curl(info):
    return curl0(info, [bestbox1(info), mbedtls1_dev(info), make1(info), tar1(info), xz1(info), musl1(info)], 'xz')
